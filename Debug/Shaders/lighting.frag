...
struct SpotLight
{
    vec3 position;
    vec3 direction;
    float cutOff;//Cono adentro
    float outerCutOff;//Cono afuera
...
void main( )
{
    ...
    vec3 result = CalcDirLight( dirLight, norm, viewDir );//Hacia donde van las luces
...
vec3 CalcDirLight( DirLight light, vec3 normal, vec3 viewDir )
{
	...
	/*vec4 result= vec4(ambient + diffuse + specular,texture( material.diffuse, TexCoords).a) ;//Descarta lo que todo se hace transparente
	  if(result.a < 0.1)
        discard;*/
